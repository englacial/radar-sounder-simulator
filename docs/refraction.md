...
*The two-point solve at one interface. The objective is strictly convex, so the crossing lies in the vertical plane through `p` and `q`; the solve reduces to a 1-D Newton iteration on `sin θ` in the rarer medium (singularity-free at the critical angle) with a fixed iteration count, which keeps it JAX-compilable and exact to float precision. (`soundersim.refraction.snell_crossing` is the reference form; the multilayer kernel's sequential path runs the same iteration as a component-form `fori_loop`, `kernels/multilayer._snell_c`, which is what lets XLA fuse it — results agree to float64 rounding.) The true surface is wavy (thin grey); the solve uses that facet's flat **local plane** (dashed) — see [what stays approximate](#what-stays-approximate-either-way).*
