"""Measured vs fixture vs B1 tables from the pilot metrics.json files.
    ...
    uv run python claude_notes/roughness_b1/tabulate.py [case ...]

Default cases: pilot_smoke (fixture), pilot_smoke_b1, pilot_smoke_b1_th20/40.
"""
import json
import sys
...
CASES = sys.argv[1:] or ["pilot_smoke", "pilot_smoke_b1", "pilot_smoke_b1_th20", "pilot_smoke_b1_th40"]
    ...
    for line, ldir in LINES.items():
        ...
        for k in passes:
            ...
            for c in cols:
                ...
                cells.append(f"{sr['sigma_m'] * 100:.2f} / {sr['corr_length_m']:.3f}")
